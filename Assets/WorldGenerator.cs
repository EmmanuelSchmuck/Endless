...
public class WorldGenerator : MonoBehaviour
{
	...
	void Update(){
		if(Input.GetKeyDown(KeyCode.C)) CleanObjects();
	...
	public void AssignColor(float bonusAProportion, Color colorA, Color colorB)
	{
		Color color;
		bool isColorA;
		foreach (GameObject bo in bonus)
		{
			isColorA = Random.value > bonusAProportion;
			color =  isColorA ? colorA : colorB;
			bo.GetComponentInChildren<Renderer>().material.SetColor("_EmissionColor", color);
			bo.GetComponentInChildren<Renderer>().material.color = color;
			bo.GetComponent<Bonus>().isColorA = isColorA;
			bo.GetComponent<Light>().color = color;
			//bo.GetComponent<Bonus>().flashColor = color;
			//Debug.Log("gjreigij");
		}
	}
